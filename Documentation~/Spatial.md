...
Import the separate **Relative world** sample, open `RelativeWorld.unity`, and press Play. The scene creates an isolated realm, camera and two simple car manifestations. `SimulatedGeoSdk` returns one complete snapshot containing continuously updated `origin` and `target` readings. A one-generic `PollingPresenceDetector<GeoPoseReading>` identifies each reading and forwards raw SDK data to the Realm.
