...
The package passed in Unity **2022.3.62f3** on **2026-09-26**, package **0.1.0**, with a single-file `Realm` delegating entity lifetime to `Population` and view-request bookkeeping to `ViewManager`. All **126 tests** passed with no failures or skips. The added public-behavior test verifies that detector handover retains identities until queued startup publications finish across multiple updates, then removes unreported identities even while later traffic continues. Existing coverage verifies initialization and failure handling, bounded FIFO dispatch, identity rediscovery during removal, nested scene changes and disposal during dispatch. Both test assemblies compile without access to internal production APIs.

| Editor | Test Framework | EditMode | PlayMode |
| --- | --- | --- | --- |
| 2022.3.62f3 (`96770f904ca7`) | 1.1.33 | 9 passed | 117 passed |

The ignored XML reports and logs are in `Tests/Unity~/TestResults/PopulationExtraction/`. EditMode verifies the imported samples match the package; PlayMode also covers identity retention, stale dispatch, failure cleanup, detector restart, prefab setup and the runnable sample scenes.
