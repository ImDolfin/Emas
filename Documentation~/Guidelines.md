...
| Ownership | Emas owns anchors, ghost roots, views and availability. Applications own SDK clients and prefab assets. Source cleanup unsubscribes but does not dispose a shared client. Dispose query subscriptions when their consumer stops. |
| Threading and payloads | Unity operations and mapping run on the Unity thread. Callback-source publish/remove delegates accept worker calls; copy mutable/reused SDK payloads before publishing. Order initial data with live events and undo partial subscriptions if startup throws. Custom sources must invalidate old callbacks before reattachment. |
| Failure and recovery | Read `LastError` or **Window > Emas**. Fix the cause, then replace the failed source to preserve compatible identities. Remove/add restarts tracking but deletes the removed source's ghosts. There is no automatic retry; unavailable data may be stale. |
