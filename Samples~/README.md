...
| **Callback quick start** (`Callbacks/`) | `Callbacks.unity` | SDK callbacks, removal and unsubscribe cleanup |
| **Emas sample** (`Example/`) | `Scenes/Example.unity` | Multiple sources, typed views and source replacement |
| **Relative world** (`RelativeWorld/`) | `RelativeWorld.unity` | `Spatial` roots, `Double3` positions and a ghost-following `ReferenceFrame` |
